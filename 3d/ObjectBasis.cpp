...
//デフォルトテクスチャ格納ディレクトリ
std::string ObjectBasis::kDefaultTextureDhirectoryPath = "Resource/";
...
void ObjectBasis::CompileShaderFile() {
	HRESULT result;

	//頂点シェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resource/shader/ModelVS.hlsl",//シェーダファイル名
		nullptr,
	...
	//ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resource/shader/ModelPS.hlsl",//シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,//インクルード可能にする
...
void ObjectBasis::AssembleVertexLayout() {
	//頂点レイアウト
	enum LayoutElement {
		Position,
		TEXCOORD,
...
void ObjectBasis::CreateGraphicsPipeline() {
	AssembleGraphicsPipeline();
	GenerateRootSignature();
	GeneratePipelineState();
}

void ObjectBasis::AssembleGraphicsPipeline() {
	//グラフィックスパイプライン設定
...
void ObjectBasis::GenerateRootSignature() {
	HRESULT result;
...
void ObjectBasis::GeneratePipelineState() {
	HRESULT result;
	//パイプラインステートの生成
...
void ObjectBasis::LoadTexture(uint32_t textureIndex, const std::string& fileName) {
...
void ObjectBasis::GenerateTextureBuffer(uint32_t textureIndex,const wchar_t* wfileName) {
	HRESULT result;
...
void ObjectBasis::GenerateDescriptorHeap() {
	HRESULT result;
...
void ObjectBasis::CreateShaderResourceView(uint32_t textureIndex) {
	//シェーダーリソースビュー設定
