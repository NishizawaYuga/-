...
void Stage::CheckPos(Vector3* player) {
	...
	for (int y = 0; y < blockNum; y++) {
		for (int x = 0; x < blockNum; x++) {
			...
			if (player->x - range < block[y][x].pos.position_.x && player->x + range > block[y][x].pos.position_.x) {
				if (player->y - range < block[y][x].pos.position_.y && player->y + range > block[y][x].pos.position_.y) {
